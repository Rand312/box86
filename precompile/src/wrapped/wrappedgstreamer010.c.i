# 0 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c"
...
# 3 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 4 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 5 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 6 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 8 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 10 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 11 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 12 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 13 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
...
# 14 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2

const char* gstreamer010Name = "libgstreamer-0.10.so.0";
# 26 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c"
...
int wrappedgstreamer010_init(library_t* lib, box86context_t* box86)
{
    (void)box86;
...
# 143 "/home/ssd/pico/code/box86/src/wrapped/wrappedlib_init.h"
    ...
    {
        lib->w.lib = dlopen(gstreamer010Name, 
...
# 159 "/home/ssd/pico/code/box86/src/wrapped/wrappedlib_init.h"
        ...
        } else lib->path = box_strdup(gstreamer010Name);
    }
...
# 188 "/home/ssd/pico/code/box86/src/wrapped/wrappedlib_init.h"
 cnt = sizeof(gstreamer010symbolmap)/sizeof(map_onesymbol_t); for (int i = 0; i < cnt; ++i) { if (gstreamer010symbolmap[i].weak) { k = kh_put_symbolmap(lib->w.wsymbolmap, gstreamer010symbolmap[i].name, &ret); ((lib->w.wsymbolmap)->vals[k]).w = gstreamer010symbolmap[i].w; ((lib->w.wsymbolmap)->vals[k]).resolved = 0; } else { k = kh_put_symbolmap(lib->w.symbolmap, gstreamer010symbolmap[i].name, &ret); ((lib->w.symbolmap)->vals[k]).w = gstreamer010symbolmap[i].w; ((lib->w.symbolmap)->vals[k]).resolved = 0; } if (strchr(gstreamer010symbolmap[i].name, '@')) AddDictionnary(box86->versym, gstreamer010symbolmap[i].name); }
 cnt = sizeof(gstreamer010mysymbolmap)/sizeof(map_onesymbol_t); for (int i = 0; i < cnt; ++i) { if (gstreamer010mysymbolmap[i].weak) { k = kh_put_symbolmap(lib->w.wmysymbolmap, gstreamer010mysymbolmap[i].name, &ret); ((lib->w.wmysymbolmap)->vals[k]).w = gstreamer010mysymbolmap[i].w; ((lib->w.wmysymbolmap)->vals[k]).resolved = 0; } else { k = kh_put_symbolmap(lib->w.mysymbolmap, gstreamer010mysymbolmap[i].name, &ret); ((lib->w.mysymbolmap)->vals[k]).w = gstreamer010mysymbolmap[i].w; ((lib->w.mysymbolmap)->vals[k]).resolved = 0; } if (strchr(gstreamer010mysymbolmap[i].name, '@')) AddDictionnary(box86->versym, gstreamer010mysymbolmap[i].name); }

    cnt = sizeof(gstreamer010stsymbolmap)/sizeof(map_onesymbol_t);
    for (int i=0; i<cnt; ++i) {
        k = kh_put_symbolmap(lib->w.stsymbolmap, gstreamer010stsymbolmap[i].name, &ret);
        ((lib->w.stsymbolmap)->vals[k]).w = gstreamer010stsymbolmap[i].w;
        ((lib->w.stsymbolmap)->vals[k]).resolved = 0;
        if(strchr(gstreamer010stsymbolmap[i].name, '@'))
            AddDictionnary(box86->versym, gstreamer010stsymbolmap[i].name);
    }
    cnt = sizeof(gstreamer010symbol2map)/sizeof(map_onesymbol2_t);
    for (int i=0; i<cnt; ++i) {
        k = kh_put_symbol2map(lib->w.symbol2map, gstreamer010symbol2map[i].name, &ret);
        ((lib->w.symbol2map)->vals[k]).name = gstreamer010symbol2map[i].name2;
        ((lib->w.symbol2map)->vals[k]).w = gstreamer010symbol2map[i].w;
        ((lib->w.symbol2map)->vals[k]).weak = gstreamer010symbol2map[i].weak;
        ((lib->w.symbol2map)->vals[k]).resolved = 0;
        if(strchr(gstreamer010symbol2map[i].name, '@'))
            AddDictionnary(box86->versym, gstreamer010symbol2map[i].name);
    }
    cnt = sizeof(gstreamer010datamap)/sizeof(map_onedata_t);
    for (int i=0; i<cnt; ++i) {
        if(gstreamer010datamap[i].weak) {
            k = kh_put_datamap(lib->w.wdatamap, gstreamer010datamap[i].name, &ret);
            ((lib->w.wdatamap)->vals[k]) = gstreamer010datamap[i].sz;
        } else {
            k = kh_put_datamap(lib->w.datamap, gstreamer010datamap[i].name, &ret);
            ((lib->w.datamap)->vals[k]) = gstreamer010datamap[i].sz;
        }
    }
    cnt = sizeof(gstreamer010mydatamap)/sizeof(map_onedata_t);
    for (int i=0; i<cnt; ++i) {
        k = kh_put_datamap(lib->w.mydatamap, gstreamer010mydatamap[i].name, &ret);
        ((lib->w.mydatamap)->vals[k]) = gstreamer010mydatamap[i].sz;
    }

    lib->w.altmy = box_strdup("my010_"); setNeededLibs(lib, 1, "libglib-2.0.so.0");


    return 0;
}

void wrappedgstreamer010_fini(library_t* lib)
{
...
# 27 "/home/ssd/pico/code/box86/src/wrapped/wrappedgstreamer010.c" 2
